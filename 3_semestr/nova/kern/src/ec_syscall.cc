...
*/
            ...
            if(r->esi < Ec::break_current){
                ...
                //     printf("here1\n");
                //     //memset(reinterpret_cast<void*>(Ec::break_current), 0, align_up(Ec::break_current, PAGE_SIZE)-1);
                //     printf("a tohle?\n");
                //     Ec::break_current = r->esi;
                //     printf("return 4\n");
                //     r->eax = original_break;
                ...
                /*	mword vaddr = align_up(Ec::break_current, PAGE_SIZE);
					if(vaddr != Ec::break_current){
						if(r->esi < vaddr){
							memset(reinterpret_cast<void*>(break_current),0, r->esi-break_current);
						} else {
							memset(reinterpret_cast<void*>(break_current),0, vaddr-break_current);
						}
					}
					mword startAlloc = align_up(original_break, PAGE_SIZE);
                	while(r->esi > startAlloc){
						void* ret = Kalloc::allocator.alloc_page (1, Kalloc::FILL_0);
						if(ret == NULL){
							while(startAlloc>Ec::break_current){
	                            //printf("chybka\n");
	                            //mword tofree = align_dn(Ptab::get_mapping(vaddr), PAGE_SIZE);
	                            Ptab::insert_mapping(startAlloc, 0,0);
	                            Kalloc::allocator.free_page(Kalloc::phys2virt(align_dn(Ptab::get_mapping(startAlloc-PAGE_SIZE), PAGE_SIZE)));
                ...
                //printf("AAAAA r->esi: %lx\n... align_up: %lx\n Ec::break_current: %lx\n", r->esi, align_up(Ec::break_current, PAGE_SIZE),Ec::break_current);
                while(startAlloc < r->esi){
                    void* ptr = Kalloc::allocator.alloc_page (1, Kalloc::FILL_0);
                    if( ptr && Ptab::insert_mapping (startAlloc, Kalloc::virt2phys(ptr), Ptab::PRESENT | Ptab::RW | Ptab::USER )){
                    } else {
                        //printf("Oh shit, oh fuck error\n");
                        while(startAlloc > Ec::break_current){
                            mword temp = Ptab::get_mapping(startAlloc-PAGE_SIZE);
                            Ptab::insert_mapping(startAlloc, 0, 0);
                            Kalloc::allocator.free_page(Kalloc::phys2virt(align_dn(temp, PAGE_SIZE)));
                            startAlloc -= PAGE_SIZE;
                        }
                        r->eax = 0;
                        printf("return 4\n");
