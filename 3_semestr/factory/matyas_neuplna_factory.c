    ...
    while(true){
        ...
        if(ready_places[worker->x] > 0) {
            ...
            for (int i = _PHASE_COUNT -1; i >= 0; --i){
               ...
               // printf("%d parts 0 0\n", parts[0][0]);
                if(!working && !reading){
                    if(parts[A][i] == 1 && (i+1) != 6 && !working && (parts[B] == 0 && parts[C] == 0)){
                       // printf("parts check\n");
                       // printf("%d i\n", i);
                       // printf("%d parts +1 check\n", parts[A][i+1]);
                        ...
                        if(parts[A][i+1] == 0 && occupiedWs[makeA[i+1]] < 1){
                            ...
                            cantWork = true; //can work or check with field of objects  we are making
                            pthread_cond_signal(&cond);
                        }
                    }
                    if(parts[B][i] == 1 && (i+1) != 6 && !working && (parts[A] == 0 && parts[C] == 0)){
                       // printf("parts check\n");
                       // printf("%d parts +1 check\n", parts[A][i+1]);
                    ...
                    if(parts[C][i] == 1 && (i+1) != 6 && !working && (parts[B] == 0 && parts[A] == 0)){
                       // printf("parts check\n");
                       // printf("%d parts +1 check\n", parts[A][i+1]);
                       // printf("%d readzy places check\n", ready_places[makeA[i+1]]);
...
int main(int argc, char **argv) {
 ...
 //HERE?                //printf("getout of here worker %d\n", i);
