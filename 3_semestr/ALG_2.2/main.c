...
void quicksort(experiment_t **number,int first,int last){
	int i, j, pivot;
	experiment_t *temp;
	if(first<last){
		pivot=first;
		...
		j=last;
		while(i<j){
			while(number[i]->requieredDays<=number[pivot]->requieredDays&&i<last)
				i++;
				while(number[j]->requieredDays>number[pivot]->requieredDays)
				j--;
				if(i<j){
				temp=number[i];
				number[i]=number[j];
...
{	
	...
	/*for (int i = 0; i < numberOfExperiments; ++i)
	{
		printf("%i ", originalExperiments[i]->requieredDays);
	}
	printf("\n");*/

	//quicksort(originalExperiments, originalExperiments[0]->requieredDays, originalExperiments[numberOfExperiments-1]->requieredDays);

	

	for (int i = 0; i < numberOfExperiments-1; ++i)
	{
		for (int j = 0; j < numberOfExperiments-i-1; ++j)
		{
			if (originalExperiments[j]->requieredDays<originalExperiments[j+1]->requieredDays)
			{
				experiment_t *temp = originalExperiments[j];
				//printf("%i\n", temp->income);
				originalExperiments[j] = originalExperiments[j+1];
				originalExperiments[j+1] = temp;
			}
		}
	}
	/*for (int i = 0; i < numberOfExperiments; ++i)
...
int func (int* schedule, int numberOfRemainingExperiments, int currentBest, int top, experiment_t** experiments){
	...
	for(int experimentNO = 0; experimentNO < numberOfRemainingExperiments; experimentNO++){
		//again:
		//printf("%i\n", experimentNO);
		tempSched = (int*)malloc(numberOfDays * sizeof(int));
		/*nextExperiment:
