...
int main(int argc, char const *argv[]){
	long int numberOfPlanes;
	int pilotWeight;
	scanf("%li %i", &numberOfPlanes, &pilotWeight);
...
long int placePils(node* root, int pilotWeight, int imbalance, int NumberOfPilots){
	...
	int addPilots = imbalance-root->balance+abs(root->rightWeight-(root->leftWeight+NumberOfPilots*pilotWeight));
	int temp = placePils(root->left, pilotWeight, addPilots, NumberOfPilots);
	if(temp < possibleBest){
		possibleBest = temp;
	}

	addPilots = imbalance-root->balance+abs(-root->rightWeight+(root->leftWeight+NumberOfPilots*pilotWeight));
	temp = placePils(root->right, pilotWeight, addPilots, NumberOfPilots);
	if(temp < possibleBest){
		possibleBest = temp;
	}
	if(NumberOfPilots-1>0){
		temp = placePils(root->right, pilotWeight, imbalance, NumberOfPilots-1) + placePils(root->left, pilotWeight, imbalance, NumberOfPilots-1);
		if(temp < possibleBest){
			possibleBest = temp;
		}
	}
	return possibleBest+imbalance;
}
...
*/
	...
	int i = 1;
	int j = 1;
	long int leftWeight = planes[0];
	long int rightWeight = planes[numberOfPlanes-1];
