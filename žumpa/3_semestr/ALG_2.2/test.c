...
int func (int* schedule, int experimentNO, int currentBest, int top, experiment_t** experiments){
	int blob=0;
	printf("AAAAAAAAAAAAAAA %i\n", experiments[experimentNO]->activeDays[experiments[experimentNO]->requieredDays-1]);
	for (int i = 0; i < experiments[experimentNO]->activeDays[experiments[experimentNO]->requieredDays-1]+i < numberOfDays; ++i){
		for (int k = 0; k < experiments[experimentNO]->requieredDays; k++){\
			printf("DOSTANU SE SEM?\n");
			if(schedule[(experiments[experimentNO]->activeDays[k])+i]!=0){
				break;	
			} else {
				blob++;
			}
		...
		if(blob==experiments[experimentNO]->requieredDays){
			for (int k = 0; k < experiments[experimentNO]->requieredDays; k++){
				schedule[(experiments[experimentNO]->activeDays[k])]=experimentNO+1;
			}
		}
		blob=0;
		for (int g = 0; g < numberOfDays; ++g)
		{
			printf("%i ",schedule[i]);
		}
		printf("\n");
